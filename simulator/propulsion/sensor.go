...
// DefaultPropellantSensor возвращает характеристики датчика остатка
// компонента в баке.
//
// В отличие от давления или оборотов, здесь нет прямого физического датчика
// массы — остаток оценивается по времени работы, перепаду давления наддува
// или счётчику расхода, и такая оценка инерционна и склонна к систематическому
// смещению (протечка калибровки, всплески при манёвре), а не к быстрому шуму:
// отсюда постоянная времени на два порядка больше, чем у датчика давления,
// и дрейф заметно выше. Именно из-за этой инерционности решения по остатку
// топлива (MECO, довыведение) не могут опираться на показание один в один —
// запас всегда рассчитывается так, чтобы пережить умеренную ошибку оценки.
func DefaultPropellantSensor(fullScale float64) SensorConfig {
	return SensorConfig{
		NoiseFraction:   0.003,
		DriftRate:       1.5e-5,
		FullScale:       fullScale,
...
//
// Внутреннее состояние стартует с нуля — это верно для величин, чьё истинное
// значение тоже нулевое до старта (давление в камере, обороты вала до
// запуска двигателя): переходный процесс от нуля к истинному значению в
// этом случае неотличим от реальности. Если истинное значение при старте
// симуляции заведомо не близко к нулю (например, почти полный бак), нужен
// NewSensorAt — иначе первые секунды датчик показывал бы паразитный
// переходный процесс, а не реальный отсчёт.
func NewSensor(cfg SensorConfig) *Sensor {
	return &Sensor{Config: cfg}
}

// NewSensorAt создаёт датчик, уже показывающий заданное значение.
func NewSensorAt(cfg SensorConfig, initial float64) *Sensor {
	return &Sensor{Config: cfg, filtered: initial, held: initial, valid: true}
}
