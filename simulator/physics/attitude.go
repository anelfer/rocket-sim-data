...
// AngleUnwrapper накапливает НЕПРЕРЫВНЫЙ угол из циклических показаний.
//
// Азимут выдаётся в диапазоне 0…360, крен — в −180…180. На графике оба
// разрываются там, где физического разрыва нет: корпус, равномерно
// вращающийся через север, рисуется отвесной линией с 359° на 1°, и по
// такому графику невозможно ни увидеть скорость вращения, ни отличить
// плавный проход границы от настоящего рывка.
//
// Разворачивание снимает разрыв: к накопленному значению прибавляется
// КРАТЧАЙШАЯ разность с предыдущим показанием (ShortestAngle), поэтому
// 359° → 1° продолжается как 359° → 361°, а 179° → −179° как 179° → 181°.
// Величина перестаёт быть углом в обычном смысле — она может уйти за
// пределы оборота, и это ровно то, что нужно показать: обороты видны.
//
// Разворачивание переживает разрывы больше половины оборота ЗА ОДИН ШАГ
// неправильно — такой скачок неотличим от поворота в обратную сторону.
// Ограничение принципиальное, не реализации: при частоте телеметрии в
// двадцать герц это соответствует 3600 °/с, вчетверо выше разрушающей
// угловой скорости корпуса.
type AngleUnwrapper struct {
	total float64
	prev  float64
	valid bool
}

// Update принимает очередное циклическое показание (градусы) и возвращает
// накопленный непрерывный угол.
func (u *AngleUnwrapper) Update(deg float64) float64 {
	if !u.valid {
		u.total, u.prev, u.valid = deg, deg, true
		return u.total
	}
	u.total += shortestAngle(deg - u.prev)
	u.prev = deg
	return u.total
}

// Value возвращает накопленный угол, не принимая нового показания.
// До первого Update — ноль.
func (u *AngleUnwrapper) Value() float64 { return u.total }

// Reset забывает накопленное: следующий Update начнёт отсчёт заново.
func (u *AngleUnwrapper) Reset() { *u = AngleUnwrapper{} }
