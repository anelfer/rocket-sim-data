...
type AttitudeControlConfig struct {
	...
	// RCSRateCommand включает закон «заказывается угловая СКОРОСТЬ»:
	// вместо обрезанной по потолку потребности ПД орган отрабатывает
	// разницу между фактической угловой скоростью и такой, которую он же
	// способен погасить на оставшемся угле. Нужен там, где двигатели
	// ориентации — единственный орган и власти заведомо не хватает
	// (пассивный участок возврата первой ступени). Аппаратам, у которых
	// они лишь помогают аэродинамическим поверхностям, не нужен и вреден:
	// см. разбор в Update, секция 6.
	RCSRateCommand bool
}
...
type VehicleAttitude struct {
	...
	// filteredRateCommand — сглаженная команда закона по скорости
	// (RCSRateCommand). Отдельно от filteredResidual: величины разные.
	filteredRateCommand physics.Vec3
}
...
func (a *VehicleAttitude) integrate(dt float64, in AttitudeInput) {
	...
	if !enginesActive && cfg.RCSMoment > 0 && !ov.Dead {
		...
		// Распределение бюджета: СНАЧАЛА гашение угловой скорости, и только
		// остатком — доворот к цели.
		//
		// Это не перестановка слагаемых, а смена того, что контур вообще
		// пытается сделать, когда власти не хватает. Прежде момент
		// назначался одним куском: потребность ПД (в ней при ошибке в сто
		// градусов подавляющая часть — пропорциональный член) обрезалась по
		// потолку газоотвода. Замерено на пассивном участке возврата:
		// потребные 56 МН·м против располагаемых 0.354 — обрезка оставляла
		// чистый «туда», выдаваемый на пределе целую минуту. Момент при
		// этом не удерживал корпус, а РАСКРУЧИВАЛ его: 0.354 МН·м на
		// инерции 1.6·10⁸ кг·м² дают 0.13 °/с², и за пятьдесят пять секунд
		// набегали те самые 5 °/с, с которыми ступень входила в атмосферу
		// в произвольном положении.
		//
		// Физически это разные задачи, и при дефиците власти они не
		// равноправны. Ошибку в сто градусов таким органом не закрыть ни
		// за какое время — а вот не дать накопиться угловой скорости он
		// способен: на гашение 5 °/с нужно сорок секунд работы, и это
		// выполнимо.
		//
		// Но «сначала гашение, остальное на доворот» как СУММА ДВУХ
		// СЛАГАЕМЫХ не работает, и это проверено: при ω = 1 °/с и ошибке
		// 80° демпфирующая часть давала −125 кН·м, доворотная забирала
		// оставшиеся +125, и в сумме выходил ноль. Два куска одного
		// бюджета, направленные навстречу, просто гасят друг друга —
		// корпус уходил от цели с постоянной скоростью при якобы
		// работающем управлении.
		//
		// Приоритет задаётся не дележом момента, а тем, ЧТО именно
		// заказывается. Заказывается угловая СКОРОСТЬ, и заказывается
		// такая, которую этот же орган способен погасить на оставшемся
		// угле: ω_зад = √(2·α·|ошибка|) с запасом, но не быстрее
		// rcsMaxSlewRate. Момент отрабатывает разницу между заказанной и
		// фактической скоростью. Отсюда оба нужных поведения выходят
		// сами: если корпус вращается быстрее, чем можно остановить, вся
		// власть идёт против вращения (это и есть приоритет гашения);
		// если медленнее — остаток разгоняет доворот к цели.
		alphaOf := func(budget, inertia float64) float64 {
			if inertia <= 0 {
				return 0
			}
			return budget / inertia
		}
		rateCommand := func(err, budget, inertia float64) float64 {
			alpha := alphaOf(budget, inertia)
			if alpha <= 0 {
				return 0
			}
			// Запас на торможение: тормозить приходится тем же моментом,
			// который в этот момент может отбирать аэродинамика.
			const brakeMargin = 0.5
			want := math.Sqrt(2 * brakeMargin * alpha * math.Abs(err))
			if want > rcsMaxSlewRate {
				want = rcsMaxSlewRate
			}
			if err < 0 {
				return -want
		...
		command := func(err, omega, surface, budget, inertia float64) float64 {
			target := rateCommand(err, budget, inertia)
			want := inertia * (target - omega) / rcsRateTime
			return clampAbs(want-surface, budget)
		}

		// Переключение по НАСЫЩЕНИЮ, поканально.
		//
		// Пока потребность ПД помещается в располагаемый момент, орган
		// отрабатывает её как есть — это и есть нормальная работа контура,
		// и трогать её незачем: у корабля двигатели ориентации сопоставимы
		// с его инерцией, они успевают и доворачивать, и гасить. Замерено:
		// закон по скорости, включённый там БЕЗУСЛОВНО, замедлял корабль
		// настолько, что угол атаки на входе уходил на 107°, а посадка
		// становилась аварийной.
		//
		// Другое дело — канал, где потребность больше располагаемого. Там
		// обрезка по модулю означает «выдать всю власть в сторону цели»,
		// и именно она раскручивала бустер: цель недостижима, а момент
		// всю минуту идёт в одну сторону. В этом случае и включается закон
		// по скорости.
		// Какой закон применять, решает НАСТРОЙКА АППАРАТА, а не режим
		// полёта, и это не уклонение от общего решения — это признание
		// того, что у двух аппаратов двигатели ориентации занимают разное
		// место в системе управления.
		//
		// У корабля они — помощник: корпус на входе держат плавники, а
		// двигатели добавляют в сторону цели то немногое, что могут. Их
		// потребность превышает располагаемое постоянно и в десятки раз,
		// и это нормальная работа, а не авария. Замерено: закон по
		// скорости, включённый и там, уводил угол атаки на 110° и
		// заставлял двигатели молотить в плотных слоях, где работать
		// обязаны плавники.
		//
		// У бустера на пассивном участке они — единственный орган:
		// двигатели не работают, рули в разрежённых слоях не дают ничего.
		// Там выдача всей власти в сторону недостижимой цели не приближает
		// к ней, а копит угловую скорость (56 МН·м потребных против 0.354
		// располагаемых, |ω| росла 0.5 → 5 °/с за минуту), и заказывать
		// надо скорость, а не момент.
		choose := func(residual, byRate, budget float64) float64 {
			if !cfg.RCSRateCommand || math.Abs(residual) <= budget {
				return clampAbs(residual, budget)
			}
			return byRate
		}

		limitRoll := cfg.RCSMoment * cfg.RollAuthority * 10

		// Команда по скорости сглаживается так же и по той же причине, что
		// и потребность ПД выше: она строится из ОШИБКИ ОРИЕНТАЦИИ и
		// УГЛОВОЙ СКОРОСТИ, то есть из показаний приборов, и сырой шум в
		// них заставляет сопла молотить на каждом такте. Своё состояние, а
		// не общее с filteredResidual: у двух законов разные величины, и
		// подмешивать одну в другую при переключении нельзя.
		rateCmd := physics.Vec3{
			X: command(a.AttitudeError.X, a.Omega.X, a.SurfaceTorque.X, limitRoll, a.Inertia.Ixx),
			Y: command(a.AttitudeError.Y, a.Omega.Y, a.SurfaceTorque.Y, cfg.RCSMoment, a.Inertia.Iyy),
			Z: command(a.AttitudeError.Z, a.Omega.Z, a.SurfaceTorque.Z, cfg.RCSMoment, a.Inertia.Izz),
		}
		if a.SensedOrientation.Norm() > 1e-9 && dt > 0 {
			const rateFilterTime = 1.5
			decay := math.Exp(-dt / rateFilterTime)
			a.filteredRateCommand = rateCmd.Add(a.filteredRateCommand.Sub(rateCmd).Scale(decay))
			rateCmd = a.filteredRateCommand
		} else {
			a.filteredRateCommand = rateCmd
		}

		a.ControlTorque = physics.Vec3{
			X: choose(residual.X, rateCmd.X, limitRoll),
			Y: choose(residual.Y, rateCmd.Y, cfg.RCSMoment),
			Z: choose(residual.Z, rateCmd.Z, cfg.RCSMoment),
		}
...
// Пределы контура двигателей ориентации.
//
// rcsMaxSlewRate — потолок ЗАКАЗЫВАЕМОЙ угловой скорости, рад/с (2 °/с).
// Быстрее корпус на таком органе разворачивать нельзя не потому, что он не
// разгонится, а потому, что его потом нечем будет остановить: тем же
// моментом, тем же плечом и на той же инерции торможение стоит ровно
// столько же, сколько разгон.
//
// rcsRateTime — постоянная времени отработки заказанной скорости, с.
// Момент назначается как I·(ω_зад−ω)/rcsRateTime, поэтому при большой
// разнице команда сама упирается в располагаемый момент — отдельного
// «режима гашения» не требуется.
const (
	rcsMaxSlewRate = 2 * physics.DegToRad
	rcsRateTime    = 5.0
)
