...
func firstStageEngineLayout(cfg Config) []EngineLayout {
	...
	for i, n := range counts {
		// Качаются только двигатели внутреннего кольца: у сверхтяжёлого
		// носителя наружные закреплены жёстко, и располагаемый управляющий
		// момент определяется именно центральной группой.
		specs = append(specs, ringSpec{
			Count:  n,
			Nozzle: nozzle,
			Kind:   "sealevel",
			Gimbal: i == 0 || len(counts) == 1,
		})
	}

	out := placeRings("S1", specs, cfg.Diameter/2)

	// Нумерация сквозная по ступени, а не по кольцу: идентификаторы обязаны
	// совпадать с теми, что выдаёт двигательная установка.
	for i := range out {
		out[i].ID = fmt.Sprintf("S1-%d", i+1)
	}
	return out
}
